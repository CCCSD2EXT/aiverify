...
const GroupDetail: React.FC = () => {
  const { checklists, selectedGroup, setSelectedChecklist } = useChecklists();
  const router = useRouter();

  // Memoize filtered checklists to prevent unnecessary re-renders
  ...
  if (!selectedGroup) {
    return (
      <div className="flex h-full items-center justify-center text-secondary-400">
        Please select or create a group to view checklists.
      </div>
    );
  }
  ...
  if (groupChecklists.length === 0) {
    return (
      <div className="flex h-full items-center justify-center text-secondary-400">
        No checklists available in this group.
      </div>
    );
  }

  return (
    <div className="scrollbar-hidden flex h-full flex-col gap-4 overflow-y-auto bg-secondary-950 p-4">
